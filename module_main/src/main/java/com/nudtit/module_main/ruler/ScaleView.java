package com.nudtit.module_main.ruler;
...
public class ScaleView extends View {
     ...
     * 文字下边距（避免显示不全）
     */
    private int mFontMarginBottom = 60;
     ...
     * 低刻度的边距（控制长度）
     */
    private int mLowPointerMargin = 90;

    /**
     * 中刻度的边距（控制长度）
     */
    private int mMiddlePointerMargin = 85;

    /**
     * 高刻度的边距（控制长度）
     */
    private int mHighPointerMargin = 80;
     ...
     * 底线颜色
     */
    private int mBaseLineColor = ContextCompat.getColor(Utils.getContext(), R.color.colorScale);

    /**
     * 高刻度线颜色
     */
    private int mHighScaleColor = ContextCompat.getColor(Utils.getContext(), R.color.colorScale);

    /**
     * 中刻度线颜色
     */
    private int mMiddleScaleColor = ContextCompat.getColor(Utils.getContext(), R.color.colorScale);

    /**
     * 低刻度线颜色
     */
    private int mLowScaleColor = ContextCompat.getColor(Utils.getContext(), R.color.colorScale);

    /**
     * 刻度数值颜色
     */
    private int mNumColor = ContextCompat.getColor(Utils.getContext(), R.color.colorText);

    /**
     * 指针颜色
     */
    private int mPointerColor = ContextCompat.getColor(Utils.getContext(), R.color.colorPointer);
    ...
    public ScaleView(Context context, @Nullable AttributeSet attrs) {
        ...
        scaleHandler=new ScaleHandler(Looper.getMainLooper(),new WeakReference<>(this));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawNum();
        //绘制底部数值
        drawBottomText(canvas, String.valueOf(mNowIndex / mScaleRatio));
    }
    ...
    public void drawBottomText(Canvas canvas, String text) {
        canvas.drawText(text, mPointerPosition, mHeight - mBaseLineMarginBottom + mTextSize, mPaint);
...
//            y = mFontMarginTop;
//        }
        onDrawLeftNum(mHeight - mMiddlePointerMargin);
        onDrawRightNum(mHeight - mMiddlePointerMargin);
        onDrawCenter(mHeight - mMiddlePointerMargin);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mPosX = event.getX();
                mPosY = event.getY();
                scaleHandler.sendEmptyMessage(WHAT_STOP);
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                inertiaScroll((int) (xVelocity * 0.5));
                break;
            default:
        }
        return true;
    ...
    protected void onDetachedFromWindow() {
        ...
        scaleHandler.removeMessages(WHAT_PLUS);
        scaleHandler.removeMessages(WHAT_MINUS);
    }
    ...
    private void inertiaScroll(int xVelocity) {
        ...
        for (int i = 0; i < count; i++) {
            scaleHandler.sendEmptyMessageDelayed(what, postDuration);
            //匀速时间
...
//            }
    ...
    public static class ScaleHandler extends Handler {
        ...
        public ScaleHandler(Looper looper, WeakReference<ScaleView> scaleViewWeakReference) {
            super(Looper.getMainLooper());
            this.scaleView = scaleViewWeakReference.get();
        }
            ...
            if (null != scaleView) {
                switch (msg.what) {
                    ...
                    case WHAT_MINUS:
                        ...
                        break;
                    case WHAT_STOP:
                        removeMessages(WHAT_PLUS);
                        removeMessages(WHAT_MINUS);
                        break;
                    default:
                        break;
                }
