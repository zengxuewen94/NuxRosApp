 ...
 */
public class WebHandleView extends RelativeLayout implements AnimationListener {
    ...
    public WebHandleView(Context context) {
        super(context);
        initVirtualJoystick(context);
    }

    public WebHandleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initVirtualJoystick(context);
    }

    public WebHandleView(Context context, AttributeSet attrs, int defStyle) {
...
//            @Override
//            public void run() {
//                onMoveListener.onMove(cmdLinear, cmdAngular);
//            }
//        };
//        cmdTimer.schedule(cmdTask, 0, 300L);
        isStart = true;
        moveRunnable = new MoveRunnable();
        ThreadPoolManager.getInstance().cancelAll();
        ThreadPoolManager.getInstance().execute(moveRunnable);
    ...
    private void onContactMove(float x, float y) {
        ...
        cmdLinear = normalizedMagnitude * Math.cos(contactTheta * Math.PI / 180.0);
        cmdAngular = normalizedMagnitude * Math.sin(contactTheta * Math.PI / 180.0);
...
//        }
        isStart = false;
        if (null != moveRunnable) {
            ThreadPoolManager.getInstance().cancel(moveRunnable);
        }
        cmdAngular = 0;
        cmdLinear = 0;
        endTurnInPlaceRotation();
    ...
    private TimerTask cmdTask;
    private double cmdLinear, cmdAngular;
