package com.eaibot.running.widget;
...
public class RoundMenuView extends View {
    ...
    private OnMenuClickListener onCoreClickListener;//中心按钮的点击回调
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (roundMenus != null && roundMenus.size() > 0) {
            ...
            for (int i = 0; i < roundMenus.size(); i++) {
                ...
                matrix.postTranslate((float) ((coreX + getWidth() / 2 * roundMenu.iconDistance) - (roundMenu.icon2.getWidth() / 2)), coreY - (roundMenu.icon2.getHeight() / 2));
                matrix.postRotate(((i + 1) * sweepAngle), coreX, coreY);
                canvas.drawBitmap(onClickState == i?roundMenu.icon2:roundMenu.icon, matrix, null);
...
//            }
    ...
    float mLastMotionX, mLastMotionY, xUp;

    @SuppressLint("ClickableViewAccessibility")
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        OnMenuClickListener onClickListener = null;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (!mIsLongPressed &&onClickListener!=null) {
                    onClickListener.onClick(onClickState);
    ...
    public void setCoreMenu(int coreMenuColor, int coreMenuSelectColor, int coreMenuStrokeColor,
                            int coreMenuStrokeSize, double radiusDistance, Bitmap bitmap, OnMenuClickListener onClickListener) {
        isCoreMenu = true;
    ...
    public static class RoundMenu {
        ...
        public Bitmap icon2;//菜单的图片
        public OnMenuClickListener onClickListener;//点击监听
